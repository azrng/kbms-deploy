import{_ as e}from"./plugin-vue_export-helper-DlAUqK2U.js";import{o as i,c as d,d as n}from"./app-D8HBJYTp.js";const a={},t=n(`<h2 id="_1-开始初始化" tabindex="-1"><a class="header-anchor" href="#_1-开始初始化"><span>1. 开始初始化</span></a></h2><p>大家都知道，在开始使用 git 时，我们首先要做的就是执行 <code>git init</code>。这个指令会显示出一个我们都很熟悉的提示，特别是对于那些经常启动但又很快放弃的项目。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>Initialized empty Git repository in /home/meain/dev/src/git-talk/.git/
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>接下来，我们来探索一下 <code>.git</code> 仓库里都有些什么。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ tree .git
...
└── refs
    ├── heads
    └── tags
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如上所示，创建了多个文件和文件夹。它们分别扮演什么角色呢？下面我们来逐一了解。</p><ul><li><code>config</code> 是一个 txt 文件，里面记录了当前仓库的 git 设置，如作者信息、文件模式等。</li><li><code>HEAD</code> 表示仓库的当前 head。根据你设置的默认分支，它可能是 <code>refs/heads/master</code> 或 <code>refs/heads/main</code> 或其他你设定的名字。实际上，它指向 <code>refs/heads</code> 这个文件夹，并关联了一个名为 <code>master</code> 的文件，但该文件目前还不存在。只有在你完成首次提交后，<code>master</code> 文件才会生成。</li><li><code>hooks</code> 是一个特殊的目录，其中包含了可以在 git 执行任何操作前后运行的脚本。如果你对此感兴趣，我在这里写了一篇更详细的文章，介绍了 git 钩子的工作方式。(https://blog.meain.io/2019/making-sure-you-wont-commit-conflict-markers/)</li><li><code>objects</code> 存放的是 git 的对象，比如关于仓库中的文件、提交等的数据。我们稍后会对此进行深入探讨。</li><li><code>refs</code> 正如我们之前提到的，是用来存放引用的目录。例如，<code>refs/heads</code> 里存放的是分支的引用，而 <code>refs/tags</code> 则存放的是标签的引用。我们将进一步深入了解这些文件的内容。</li></ul><h3 id="_1-1-加入一个新文件的操作" tabindex="-1"><a class="header-anchor" href="#_1-1-加入一个新文件的操作"><span>1.1 加入一个新文件的操作</span></a></h3><p>了解了 <code>.git</code> 中的初始文件集后，我们来进行第一个操作,将内容添加到 <code>.git</code> 目录。现在我们将创建并加入一个文件（此刻还未提交）。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>echo &#39;meain.io&#39; &gt; file
git add file
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>执行后，变动如下：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>--- init       2023-07-02 15:14:00.584674816 +0530
...
+│   │   └── 5b58f323d7b459664b5d3fb9587048bb0296de
 ...
 │   └── pack
 └── refs
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>此操作主要引发了两个变化。首先，文件 <code>index</code> 被修改。index 是记录当前暂存信息的地方，这表明名为 <code>file</code> 的文件已经被加入到索引中。</p><p>更为关键的是，新建了一个 <code>objects/4c</code> 文件夹，并在其中添加了 <code>5b58f323d7b459664b5d3fb9587048bb0296de</code> 文件。</p><h3 id="_1-2-这个文件里都保存了什么内容" tabindex="-1"><a class="header-anchor" href="#_1-2-这个文件里都保存了什么内容"><span>1.2 这个文件里都保存了什么内容？</span></a></h3><p>为了深入理解 <code>git</code> 的存储机制，我们先来看看这个文件具体包含了什么信息。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ file .git/objects/4c/5b58f323d7b459664b5d3fb9587048bb0296de
.git/objects/4c/5b58f323d7b459664b5d3fb9587048bb0296de: zlib compressed data
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>那么，这个用 zlib 压缩的数据中具体包含了什么呢？</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ zlib-flate -uncompress &lt;.git/objects/4c/5b58f323d7b459664b5d3fb9587048bb0296de
blob 9\\0meain.io
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>从结果可以看出，这个文件记录了我们之前通过 <code>git add</code> 命令添加的 <code>file</code> 文件的相关信息，包括文件的类型、大小和内容。具体地说，文件类型为 <code>blob</code>，大小为 <code>9</code>，内容则是 <code>meain.io</code>。</p><h3 id="_1-3-那个文件名是如何得来的" tabindex="-1"><a class="header-anchor" href="#_1-3-那个文件名是如何得来的"><span>1.3 那个文件名是如何得来的？</span></a></h3><p>这确实是个有趣的问题。这个文件名其实是基于内容的 sha1 哈希值生成的。通过对 zlib 压缩的数据进行 <code>sha1sum</code> 处理，我们就可以得到这样的文件名。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ zlib-flate -uncompress &lt;.git/objects/4c/5b58f323d7b459664b5d3fb9587048bb0296de|sha1sum
4c5b58f323d7b459664b5d3fb9587048bb0296de
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><code>git</code> 在存储内容时，会使用内容的 <code>sha1</code> 哈希值，取其前两个字符作为文件夹名（如 <code>4c</code>），余下的部分作为文件名。这种方式是为了确保在 <code>objects</code> 文件夹中不会有过多的文件，从而使文件系统保持高效。</p><h3 id="_1-4-了解-git-cat-file" tabindex="-1"><a class="header-anchor" href="#_1-4-了解-git-cat-file"><span>1.4 了解 <code>git cat-file</code></span></a></h3><p>实际上，由于这是 git 中的一个更为重要的部分，git 提供了一个基础命令 <code>git cat-file</code>，让我们可以更直观地查看它。通过 <code>-t</code> 参数，你可以查询对象的类型；使用 <code>-s</code> 参数，你可以得知对象的大小；而 <code>-p</code> 参数则能让你直观地查看对象的具体内容。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ git cat-file -t 4c5b58f323d7b459664b5d3fb9587048bb0296de
blob
...
meain.io
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_2-开始提交" tabindex="-1"><a class="header-anchor" href="#_2-开始提交"><span>2. 开始提交</span></a></h2><p>现在我们已经了解当增加一个文件时，git 会有哪些变化，接下来，我们将通过进行&quot;提交&quot;操作来进行下一步探索。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ git commit -m &#39;Initial commit&#39;
[master (root-commit) 4c201df] Initial commit
 1 file changed, 1 insertion(+)
 create mode 100644 file
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>以下是相关的变动：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>--- init        2023-07-02 15:14:00.584674816 +0530
...
+    │   └── master
     └── tags
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>变化还真多。首先有一个新文件 <code>COMMIT_EDITMSG</code>，顾名思义，它保存了最新的提交信息。</p><p>若直接运行 <code>git commit</code> 未带 <code>-m</code> 参数，git 会启动一个编辑器并加载 <code>COMMIT_EDITMSG</code> 文件，方便用户编辑提交信息。编辑完成后，git 就采用该文件内容作为提交信息。</p><p>此外，新增了一个 <code>logs</code> 目录，git 通过它来记录所有的提交变动。在此，你可以查看所有引用（refs）及 <code>HEAD</code> 的提交记录。</p><p><code>object</code> 文件夹也发生了些变化，但首先，我希望你关注一下 <code>refs/heads</code> 目录，里面现有一个 <code>master</code> 文件。毫无疑问，这就是 <code>master</code> 分支的引用。来，我们进一步了解其中的内容。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ cat refs/heads/master
3c201df6a1c4d4c87177e30e93be1df8bfe2fe19
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>显然，它是指向了一个新的对象。我们有方法查看这类对象，接着来试试。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ git cat-file -t 3c201df6a1c4d4c87177e30e93be1df8bfe2fe19
commit
...
Initial commit
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><blockquote><p>你同样可以使用 <code>git cat-file -t refs/heads/master</code> 命令来查看。</p></blockquote><p>看起来，这是我们未曾遇见的新对象类型：<code>commit</code>。从 <code>commit</code> 的内容中，我们得知它包含了一个哈希值为 <code>62902ec0eca9faceb8fe0a9870b9b6cde75a9545</code> 的 <code>tree</code> 对象，这与我们在提交时新加的对象相似。<code>commit</code> 还显示了这次提交的作者和提交者信息，这里都是我。最后，它还展示了这次提交的信息。</p><p>接下来，让我们看一下 <code>tree</code> 对象中包含的内容。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ git cat-file -t 62902ec0eca9faceb8fe0a9870b9b6cde75a9545
tree

$ git cat-file -p 62901ec0eca9faceb8fe0a9870b9b6cde75a9545
100644 blob 4c5b58f323d7b459664b5d3fb9587048bb0296de    file
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code>tree</code> 对象中会通过其他 <code>tree</code> 和 <code>blob</code> 对象的形式呈现工作目录的状态。在这个示例中，因为我们仅有一个名为 <code>file</code> 的文件，所以你只能见到一个对象。细看的话，你会发现这个文件指向了我们在执行 <code>git add file</code> 时加入的那个初始对象。</p><p>下面展示了一个更为成熟的仓库中的 <code>tree</code> 示意。在 <code>commit</code> 对象关联的 <code>tree</code> 对象中，嵌套有更多的 <code>tree</code> 对象，用以标识不同的文件夹。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ git cat-file -p 2e5e84c3ee1f7e4cb3f709ff5ca0ddfc259a8d04
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_3-进行修改" tabindex="-1"><a class="header-anchor" href="#_3-进行修改"><span>3. 进行修改</span></a></h2><p>让我们对文件进行修改，并观察这样做的结果。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ echo &#39;blog.meain.io&#39; &gt; file
$ git commit -am &#39;Use blog link&#39;
[master 68ed5aa] Use blog link
 1 file changed, 1 insertion(+), 1 deletion(-)
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>更改内容如下：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>--- commit      2023-07-02 15:33:28.536144046 +0530
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这基本上就是创建分支的全部内容。在 <code>git</code> 中，分支是相当轻便的。另外，标签的创建也是类似的操作，但它们是被创建在 <code>refs/tags</code> 目录下。</p><p>在 <code>logs</code> 目录下也新增了一个文件，该文件用于记录与 <code>master</code> 分支类似的提交历史信息。</p><h2 id="_5-分支切换" tabindex="-1"><a class="header-anchor" href="#_5-分支切换"><span>5. 分支切换</span></a></h2><p>在 <code>git</code> 中进行分支切换实际上是让 git 获取某个提交的 <code>tree</code> 对象，并更新工作区中的文件，使其与其中记录的状态相匹配。在此例中，由于我们是从 <code>master</code> 分支切换到 <code>fix-url</code> 分支，而这两个分支都指向同一个 <code>commit</code> 和它的 <code>tree</code> 对象，因此 <code>git</code> 在工作区的文件上并没有任何更改。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>git checkout fix-url
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>在进行分支切换时，<code>.git</code> 目录中唯一发生的变化是 <code>.git/HEAD</code> 文件的内容，现在它指向 <code>fix-url</code> 分支。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ cat .git/HEAD
ref: refs/heads/fix-url
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>既然我们在这里，我将进行一个提交操作。这将有助于我稍后展示合并的效果。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ echo &#39;https://blog.meain.io&#39;&gt;file
$ git commit -am &#39;Fix url&#39;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_6-合并操作" tabindex="-1"><a class="header-anchor" href="#_6-合并操作"><span>6. 合并操作</span></a></h2><p>有三种主要的合并方法。</p><ol><li>最简单且直观的是快进式合并。这种方式中，你只是更新一个分支的提交，使其指向另一个分支的提交。具体操作就是把 <code>refs/heads/fix-url</code> 中的哈希值复制到 <code>refs/heads/master</code>。</li><li>第二种是变基（rebase）合并。在这种方式中，我们首先将更改依次应用到主分支当前的提交上，然后进行类似于快进式的合并。</li><li>第三种是通过创建一个独立的合并来合并两个分支。这种方法与前两者略有不同，因为它的提交对象会有两个 <code>parent</code> 条目。我们稍后会详细探讨这种方法。</li></ol><p>首先，我们来看看合并前的 graph。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>git log --graph --oneline --all
* 42c6318 (fix-url) Fix url
* 67ed5aa (HEAD -&gt; master) Use blog link
* 3c201df Initial commit
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接下来进行合并：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ git merge fix-url # updates refs/heads/master to the hash in refs/heads/fix-url
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>我们再来看看合并后的 graph。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ git log --graph --oneline --all
* 42c6318 (HEAD -&gt; master) (fix-url) Fix url
* 67ed5aa Use blog link
* 3c201df Initial commit
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_7-执行推送" tabindex="-1"><a class="header-anchor" href="#_7-执行推送"><span>7. 执行推送</span></a></h2><p>在我们对本地 <code>git</code> 仓库进行了一系列操纵之后，现在我们来看看进行推送时会发生什么事情。远程 <code>git</code> 仓库会接收哪些数据？</p><p>为了演示这个过程，我首先创建了一个新的 <code>git</code> 仓库作为这个仓库的远程连接。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ mkdir git-talk-2
$ cd git-talk-2 &amp;&amp; git init --bare

$ cd ../git-talk &amp;&amp; git remote add origin ../git-talk-2
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>另外，添加新的远程仓库实际上是修改了配置文件，你可以在 <code>.git/config</code> 中查看这个变更。具体做了哪些修改，我鼓励你自己去探索。</p><p>接下来，执行推送操作。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ git push origin master
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>我们再来检查一下本地仓库发生了哪些改变。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>--- branch	2023-07-02 15:55:25.165204941 +0530
