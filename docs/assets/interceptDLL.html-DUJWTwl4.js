...
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>输入6时又返回1的段落了：</p><p>问题来了：internal方法怎么拦截？</p><p>我们不直接注释代码<code>mo = new CalNumber().GetValidNumber(mo);</code>，万一验证方法非常重要，我们只是需要修改其中部分逻辑，总体原逻辑不应该改变。</p><h3 id="_3-2-internal方法怎么拦截" tabindex="-1"><a class="header-anchor" href="#_3-2-internal方法怎么拦截"><span>3.2. internal方法怎么拦截？</span></a></h3><p>新增拦截类<code>HookGetValidNumber</code>，现在不能再在类上添加特性了（<code>[HarmonyPatch(typeof(CalNumber))]</code>），因为<code>CalNumber</code>不是public访问修饰，跨工程无法直接使用，语法不支持：</p><p><img src="`+l+`" alt="图片"></p><p>特性用不上，那就手工注册需要拦截的方法，这是本文的重点，代码在下面，简单提一下：</p><ul><li>手工注册代码和自动注册声明特性类似，只是换个写法；</li><li>拦截替换方法需要使用<code>HarmonyMethod</code>方法包装；</li><li><code>harmony.Patch(hookMethod, replaceHarmonyMethod);</code>用于关联被拦截方法与替换方法</li></ul><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" data-title="" style="--shiki-light:#24292e;--shiki-dark:#abb2bf;--shiki-light-bg:#fff;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes github-light one-dark-pro vp-code"><code><span class="line"><span>/// &lt;summary&gt;</span></span>
